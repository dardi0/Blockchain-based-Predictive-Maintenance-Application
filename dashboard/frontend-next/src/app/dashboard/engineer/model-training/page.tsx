...
import {
    Brain, Play, Square, RefreshCw, ArrowLeft, Settings2, BarChart3,
    ...
    return (
        <div className="space-y-1">
            ...
            {options ? (
                ...
                >
                    {options.map(o => (
                        <option key={o.value} value={o.value} className="bg-white dark:bg-slate-800 text-slate-900 dark:text-gray-100">
                            {o.label}
                        </option>
                    ))}
                </select>
                ...
                <div className="relative flex items-center">
                    <button
                        ...
                        className="absolute left-1 p-1 hover:bg-slate-200 dark:hover:bg-[var(--accent-primary)]/20 rounded text-slate-500 dark:text-slate-400 transition-colors z-10"
                    >
                        <ChevronDown size={14} />
                    </button>
                    <input
                    ...
                    <button
                        ...
                        className="absolute right-1 p-1 hover:bg-slate-200 dark:hover:bg-[var(--accent-primary)]/20 rounded text-slate-500 dark:text-slate-400 transition-colors z-10"
                    >
                        <ChevronUp size={14} />
                    </button>
                </div>
...
export default function ModelTrainingPage() {
    ...
    const [status, setStatus] = useState<TrainingStatus>({
        ...
        current_phase: '', train_loss: null, val_loss: null,
        elapsed_seconds: null, error: null,
    });

    // Results
    const [results, setResults] = useState<TrainingResults | null>(null);
    const [chartZoom, setChartZoom] = useState<string | null>(null);
    ...
    const handleStart = async () => {
        try {
            ...
            alert(e.message || 'Training could not start');
        }
    };

    // ── Helper: array param editor ─────────────────────────────────
    const ArrayParamField = ({ label, value, onChange, hint }: {
    ...
    const StatusBadge = () => {
        ...
        return (
            <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium ${c.bg} ${c.text}`}>
                <Icon size={14} className={st === 'running' ? 'animate-spin' : ''} />
                {c.label}
            </span>
        );
    };
        ...
        <div className="space-y-6 max-w-7xl mx-auto">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 dark:text-slate-400 mt-1 ml-14">
                            Configure parameters, train model, and analyze results
                        </p>
                    </div>
                </div>
                <StatusBadge />
            ...
            <div className="flex gap-1 p-1 bg-slate-100 dark:bg-[var(--dark-bg)]/80 rounded-xl w-fit">
                <button onClick={() => setActiveTab('params')}
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'params'
                        ? 'bg-white dark:bg-[var(--accent-primary)]/20 text-slate-800 dark:text-white shadow-sm'
                        : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'
                        }`}>
                    <SlidersHorizontal size={16} /> Parameters & Training
                </button>
                <button onClick={() => setActiveTab('results')}
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'results'
                        ? 'bg-white dark:bg-[var(--accent-primary)]/20 text-slate-800 dark:text-white shadow-sm'
                        : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'
                        }`}>
                    <BarChart3 size={16} /> Results & Charts
                </button>
            </div>

            {/* ═══ TAB: Parameters & Training ═══ */}
            {activeTab === 'params' && (
                <div className="space-y-6">
                    ...
                    <div className="bg-white dark:bg-[var(--dark-bg)]/90 rounded-xl border border-slate-200 dark:border-[var(--accent-primary)]/10 shadow-sm dark:shadow-[var(--accent-primary)]/5 overflow-hidden">
                        <button onClick={() => setParamsOpen(!paramsOpen)}
                            ...
                            <h2 className="text-lg font-semibold text-slate-800 dark:text-white flex items-center gap-2">
                                <Settings2 className="text-[var(--accent-primary)] dark:text-[var(--accent-highlight)]" size={20} />
                            ...
                            <div className="px-5 pb-5 space-y-6">
                                ...
                                <div>
                                    <h3 className="text-sm font-semibold text-purple-600 dark:text-purple-400 flex items-center gap-2 mb-3">
                                        <Layers size={16} /> CNN Architecture
                                    </h3>
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <ArrayParamField label="Filters / Layer" value={params.cnn_filters}
                                            onChange={v => setParams(p => ({ ...p, cnn_filters: v }))} hint="e.g. 128, 256" />
                                        <ParamField label="Kernel Size" value={params.cnn_kernel_size}
                                            onChange={v => setParams(p => ({ ...p, cnn_kernel_size: v }))} min={2} max={8} />
                                        <ParamField label="CNN Dropout" value={params.cnn_dropout}
                                            onChange={v => setParams(p => ({ ...p, cnn_dropout: v }))} min={0} max={0.8} step={0.05} />
                                    </div>
                                </div>
                                ...
                                <div>
                                    <h3 className="text-sm font-semibold text-blue-600 dark:text-blue-400 flex items-center gap-2 mb-3">
                                        <Cpu size={16} /> LSTM Architecture
                                    </h3>
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <ArrayParamField label="Units / Layer" value={params.lstm_units}
                                            onChange={v => setParams(p => ({ ...p, lstm_units: v }))} hint="e.g. 128, 256" />
                                        <ParamField label="LSTM Dropout" value={params.lstm_dropout}
                                            onChange={v => setParams(p => ({ ...p, lstm_dropout: v }))} min={0} max={0.8} step={0.05} />
                                    </div>
                                </div>
                                ...
                                <div>
                                    <h3 className="text-sm font-semibold text-emerald-600 dark:text-emerald-400 flex items-center gap-2 mb-3">
                                        <LayoutGrid size={16} /> Dense Layers
                                    </h3>
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <ArrayParamField label="Units / Layer" value={params.dense_units}
                                            onChange={v => setParams(p => ({ ...p, dense_units: v }))} hint="e.g. 32, 64, 16" />
                                        <ParamField label="Dense Dropout" value={params.dense_dropout}
                                            onChange={v => setParams(p => ({ ...p, dense_dropout: v }))} min={0} max={0.8} step={0.05} />
                                    </div>
                                </div>
                                ...
                                <div>
                                    <h3 className="text-sm font-semibold text-amber-600 dark:text-amber-400 flex items-center gap-2 mb-3">
                                        <Zap size={16} /> Optimizer & Training
                                    </h3>
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <ParamField label="Learning Rate" value={params.learning_rate}
                                            onChange={v => setParams(p => ({ ...p, learning_rate: v }))} min={0.00001} max={0.1} step={0.0001} />
                                        <ParamField label="Epochs" value={params.epochs}
                                            onChange={v => setParams(p => ({ ...p, epochs: v }))} min={10} max={2000} />
                                        <ParamField label="Batch Size" value={params.batch_size}
                                            onChange={v => setParams(p => ({ ...p, batch_size: v }))} min={8} max={512} />
                                        <ParamField label="CV Splits" value={params.cv_splits}
                                            onChange={v => setParams(p => ({ ...p, cv_splits: v }))} min={2} max={10} />
                                        <ParamField label="Early Stop Patience" value={params.early_stopping_patience}
                                            onChange={v => setParams(p => ({ ...p, early_stopping_patience: v }))} min={5} max={500} />
                                        <ParamField label="SMOTE" value={params.use_smote} type="checkbox"
                                            onChange={v => setParams(p => ({ ...p, use_smote: v }))} />
                                        <ParamField label="Threshold Method" value={params.threshold_method}
                                            onChange={v => setParams(p => ({ ...p, threshold_method: v }))}
                                            options={[
                    ...
                    <div className="bg-white dark:bg-[var(--dark-bg)]/90 rounded-xl border border-slate-200 dark:border-[var(--accent-primary)]/10 p-6 shadow-sm dark:shadow-[var(--accent-primary)]/5">
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-lg font-semibold text-slate-800 dark:text-white flex items-center gap-2">
                                <Activity className="text-[var(--accent-primary)] dark:text-[var(--accent-highlight)]" size={20} />
                                Training Control
                            </h2>
                            <div className="flex items-center gap-3">
                        ...
                        {(status.status === 'running' || status.status === 'completed') && (
                            <div className="space-y-3">
                                <div className="flex items-center justify-between text-sm">
                                    ...
                                    <span className="text-[var(--accent-primary)] dark:text-[var(--accent-highlight)] font-bold">
                                        {status.progress}%
                                    </span>
                                </div>
                                <div className="h-3 bg-slate-200 dark:bg-[var(--accent-primary)]/10 rounded-full overflow-hidden">
                                    <div
                                        className="h-full bg-gradient-to-r from-purple-500 to-[var(--accent-highlight)] rounded-full transition-all duration-700 ease-out"
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
                                    <div className="bg-slate-50 dark:bg-[var(--accent-primary)]/5 rounded-lg p-3 text-center">
                                        <p className="text-xs text-slate-400 uppercase">Epoch</p>
                                        <p className="text-lg font-bold text-slate-800 dark:text-white">
                                            {status.current_epoch} <span className="text-sm text-slate-400">/ {status.total_epochs}</span>
                                        </p>
                                    </div>
                                    <div className="bg-slate-50 dark:bg-[var(--accent-primary)]/5 rounded-lg p-3 text-center">
                                        <p className="text-xs text-slate-400 uppercase">Train Loss</p>
                                        <p className="text-lg font-bold text-blue-600 dark:text-blue-400">
                                            {status.train_loss !== null ? status.train_loss.toFixed(4) : '--'}
                                        </p>
                                    </div>
                                    <div className="bg-slate-50 dark:bg-[var(--accent-primary)]/5 rounded-lg p-3 text-center">
                                        <p className="text-xs text-slate-400 uppercase">Val Loss</p>
                    ...
                    {results ? (
                        <>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                                <MetricCard label="Test Accuracy" value={`${((results.metrics.test_accuracy || 0) * 100).toFixed(1)}%`}
                                    icon={CheckCircle} color="text-emerald-500" />
                                <MetricCard label="Test Precision" value={`${((results.metrics.test_precision || 0) * 100).toFixed(1)}%`}
                                    icon={Gauge} color="text-blue-500" />
                                <MetricCard label="Test Recall" value={`${((results.metrics.test_recall || 0) * 100).toFixed(1)}%`}
                                    icon={TrendingUp} color="text-purple-500" />
                                <MetricCard label="Test F1" value={`${((results.metrics.test_f1 || 0) * 100).toFixed(1)}%`}
                                    icon={Zap} color="text-amber-500" />
                                <MetricCard label="Test AUC" value={`${((results.metrics.test_auc || 0) * 100).toFixed(1)}%`}
                                    icon={BarChart3} color="text-[var(--accent-primary)]" />
                                <MetricCard label="Optimal Threshold" value={`${(results.metrics.optimal_threshold || 0.5).toFixed(3)}`}
                                    ...
                                    <div className="bg-white dark:bg-[var(--dark-bg)]/90 rounded-xl border border-slate-200 dark:border-[var(--accent-primary)]/10 p-5 shadow-sm">
                                        ...
                                        <div className="grid grid-cols-2 gap-2 text-center text-sm">
                                            <div className="p-3 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg">
                                                ...
                                                <p className="text-xl font-bold text-emerald-600 dark:text-emerald-400">{results.metrics.confusion_matrix.tn.toLocaleString()}</p>
                                            </div>
                                            <div className="p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
                                                <p className="text-xs text-slate-400">FP</p>
                                                <p className="text-xl font-bold text-red-500">{results.metrics.confusion_matrix.fp.toLocaleString()}</p>
                                            </div>
                                            <div className="p-3 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
                                                <p className="text-xs text-slate-400">FN</p>
                                                <p className="text-xl font-bold text-orange-500">{results.metrics.confusion_matrix.fn.toLocaleString()}</p>
                                            </div>
                                            <div className="p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                                                <p className="text-xs text-slate-400">TP</p>
                                                <p className="text-xl font-bold text-purple-600 dark:text-purple-400">{results.metrics.confusion_matrix.tp.toLocaleString()}</p>
                                            </div>
                                        </div>
                                    </div>
                                )}
                            ...
                            {results.charts.length > 0 && (
                                <div className="bg-white dark:bg-[var(--dark-bg)]/90 rounded-xl border border-slate-200 dark:border-[var(--accent-primary)]/10 p-6 shadow-sm dark:shadow-[var(--accent-primary)]/5">
                                    <h2 className="text-lg font-semibold text-slate-800 dark:text-white flex items-center gap-2 mb-6">
                                        <BarChart3 className="text-[var(--accent-primary)] dark:text-[var(--accent-highlight)]" size={20} />
                                        Training Charts ({results.charts.length})
                                    </h2>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        {results.charts.map(chart => (
                                            ...
                                            >
                                                ...
                                                <div className={`p-2 bg-white dark:bg-[var(--dark-bg)] ${chartZoom === chart.filename ? '' : ''}`}>
                                                    <img
                                                        src={`${API_URL}${chart.url}`}
                                                        alt={chart.name}
                        ...
                        <div className="bg-white dark:bg-[var(--dark-bg)]/90 rounded-xl border border-slate-200 dark:border-[var(--accent-primary)]/10 p-16 text-center shadow-sm">
                            ...
                            <p className="text-sm text-slate-400 dark:text-slate-500 mt-2">
                                Configure parameters and start training to see results here.
                            </p>
                            <button onClick={() => setActiveTab('params')}
            ...
            {chartZoom && results?.charts && (
                <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4"
                    ...
                    <div className="max-w-5xl w-full bg-white dark:bg-[var(--dark-bg)] rounded-2xl overflow-hidden shadow-2xl"
                        ...
                        <div className="px-6 py-4 bg-slate-50 dark:bg-[var(--accent-primary)]/10 flex items-center justify-between border-b border-slate-200 dark:border-[var(--accent-primary)]/10">
                            <p className="font-semibold text-slate-800 dark:text-white">
                                {results.charts.find(c => c.filename === chartZoom)?.name}
                            </p>
                            <button onClick={() => setChartZoom(null)}
                                className="p-1.5 hover:bg-slate-200 dark:hover:bg-[var(--accent-primary)]/20 rounded-lg transition-colors">
