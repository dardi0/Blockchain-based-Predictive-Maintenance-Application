...
class HybridBlockchainHandler:
    ...
    def _format_g2(point, swap=False):
        ...
        if swap:
            # Swap coordinates [x1, x0] (for some verifiers)
             return (
                [HybridBlockchainHandler._norm_value(x[1]), HybridBlockchainHandler._norm_value(x[0])],
                [HybridBlockchainHandler._norm_value(y[1]), HybridBlockchainHandler._norm_value(y[0])]
            )
        else:
             # Standard order [x0, x1]
             return (
                [HybridBlockchainHandler._norm_value(x[0]), HybridBlockchainHandler._norm_value(x[1])],
                [HybridBlockchainHandler._norm_value(y[0]), HybridBlockchainHandler._norm_value(y[1])]
            )
    ...
    def _get_signer_private_key(self, role_or_address: Optional[str] = None) -> Optional[str]:
        """
        ...
        try:
            ...
            if role_or_address and role_or_address.startswith('0x'):
                ...
                # Check admin first
                if hasattr(self, 'admin_account') and self.admin_account and self.admin_account.lower() == addr_lower:
    ...
    def _configure_role_wallets(self) -> None:
        ...
        if 'MANAGER' in self.role_wallets:
            ...
            if fallback_key:
                try:
                    address = Web3.to_checksum_address(Account.from_key(fallback_key).address)
                    info = {
                        'role': 'DEFAULT',
                        'address': address
                    }
        ...
        default_wallet = self.role_wallets.get(self.default_signer_role) if self.default_signer_role else None
        if default_wallet:
            self.admin_account = default_wallet['address']

    def _resolve_signer(self, actor_role: Optional[str], recorded_by: Optional[str]) -> Optional[Dict[str, str]]:
        ...
        if recorded_by:
            ...
            lookup_key = checksum_addr.lower() if isinstance(checksum_addr, str) else None
    ...
    def setup_session_keys(self) -> None:
        ...
        sk_artifacts_path = Path("artifacts-zk/contracts/SessionKeyAccount.sol/SessionKeyAccount.json")
        if not sk_artifacts_path.exists():
            logger.warning("setup_session_keys: SessionKeyAccount artifacts not found, skipping")
            return
        ...
        for role, selector in role_selectors.items():
            ...
            if not smart_account_addr:
                logger.info(f"setup_session_keys: no smart account for {role}, skipping")
                continue

            owner_pk = self._get_signer_private_key(role)
            if not owner_pk:
                logger.warning(f"setup_session_keys: no private key for {role}, skipping")
                continue

            try:
                ephemeral = Account.create()
                smart_account_addr_cs = self.web3.to_checksum_address(smart_account_addr)
                session_contract = self.web3.eth.contract(
                    address=smart_account_addr_cs,
                    abi=sk_abi
                )

                owner_account = Account.from_key(owner_pk)
                nonce = self.web3.eth.get_transaction_count(owner_account.address, 'pending')
                ...
                signed = self.web3.eth.account.sign_transaction(tx, private_key=owner_pk)
                tx_hash = self.web3.eth.send_raw_transaction(signed.raw_transaction)
                receipt = self.web3.eth.wait_for_transaction_receipt(
                    tx_hash, timeout=getattr(BlockchainConfig, 'TRANSACTION_TIMEOUT', 120)
                )
                if receipt.status == 1:
                    ...
                    logger.warning(f"authorizeSessionKey TX failed for {role} (status=0)")
            except Exception as e:
    ...
    def _send_as_smart_account(
        ...
        signer = PrivateKeyEthSigner(
            Account.from_key(ephemeral['private_key']),
            chain_id
        )
    ...
    def _load_local_sensor_vk_params(self):
        try:
            ...
            def _norm(v): return HybridBlockchainHandler._norm_value(v)
            alpha = [_norm(vk_json['vk_alpha_1'][0]), _norm(vk_json['vk_alpha_1'][1])]

            def _g2(point): return HybridBlockchainHandler._format_g2(point, swap=False)
    ...
    def _sensor_verifier_is_set(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Error comparing on-chain keys with local keys (Sensor): {e}")
                self._sensor_vk_ready = True
                return True
        except Exception as e:
    ...
    def _upload_sensor_verifying_key(self) -> bool:
        ...
        verifier_artifact_path = Path("artifacts-zk/contracts/UnifiedGroth16Verifier.sol/UnifiedGroth16Verifier.json")
        
        if not verifier_artifact_path.exists():
             logger.error(f"❌ UnifiedVerifier artifact not found at {verifier_artifact_path}")
        ...
        try:
            ...
            nonce = self._nonce_manager.get_nonce() if self._nonce_manager else self.web3.eth.get_transaction_count(self.admin_account, 'pending')
            ...
            gas_limit = self._gas_estimator.estimate_gas(
                base_tx, BlockchainConfig.VERIFIER_DEPLOY_GAS_LIMIT
            ) if self._gas_estimator else BlockchainConfig.VERIFIER_DEPLOY_GAS_LIMIT
            ...
            new_verifier_contract = self.web3.eth.contract(address=new_verifier_address, abi=abi)
            ...
            def _g2(p): return HybridBlockchainHandler._format_g2(p, swap=True)
            ...
            pk = self._get_signer_private_key(self.admin_account)
            signed_keys = self.web3.eth.account.sign_transaction(tx_keys, private_key=pk)
            tx_keys_hash = self.web3.eth.send_raw_transaction(signed_keys.raw_transaction)
            receipt_keys = self.web3.eth.wait_for_transaction_receipt(tx_keys_hash, timeout=60)
            ...
            if not pdm_contract: return False
            ...
            ) if self._gas_estimator else BlockchainConfig.CONTRACT_UPDATE_GAS_LIMIT
            base_tx_link['gas'] = gas_limit
            tx_link = pdm_contract.functions.updateSensorVerifier(new_verifier_address).build_transaction(base_tx_link)
            
            pk = self._get_signer_private_key(self.admin_account)
            signed_link = self.web3.eth.account.sign_transaction(tx_link, private_key=pk)
            link_tx_hash = self.web3.eth.send_raw_transaction(signed_link.raw_transaction)
            
            link_receipt = self.web3.eth.wait_for_transaction_receipt(link_tx_hash, timeout=60)
            if link_receipt.status == 1:
                logger.info(f"✅ Linked & Ready! New Sensor Verifier: {new_verifier_address}")
                self._sensor_vk_ready = True
                return True
            else:
    ...
    def _ensure_sensor_verifier_key(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                if zk_verifier_addr and zk_verifier_addr != '0x0000000000000000000000000000000000000000':
                    # Check if VK is set for CircuitType.SENSOR_DATA (0)
                    zk_verifier_abi = [{
                        "inputs": [{"internalType": "uint8", "name": "circuitType", "type": "uint8"}],
                        "name": "getICLength",
                        "outputs": [{"internalType": "uint256", "name": "", "type": "uint256"}],
                        "stateMutability": "view",
                        "type": "function"
                    }]
                    zk_verifier = self.web3.eth.contract(address=zk_verifier_addr, abi=zk_verifier_abi)
                    ic_length = zk_verifier.functions.getICLength(0).call()  # CircuitType.SENSOR_DATA = 0
                    if ic_length == 4:  # 3 public inputs + 1
                        logger.info(f"✅ Sensor VK already set on zkVerifier (IC length={ic_length})")
                        self._sensor_vk_ready = True
                        return True
            except Exception as e:
            ...
            if not current_verifier_addr or current_verifier_addr == ZERO_ADDR:
                ...
                if len(code) <= 2:
                    logger.warning(f"⚠️ Sensor Verifier address {current_verifier_addr} has no code.")
                    needs_update = True
                else:
                    ...
                    try:
                        ...
                        checker = self.web3.eth.contract(address=current_verifier_addr, abi=[{
                            "inputs": [{"internalType": "uint8", "name": "", "type": "uint8"}],
                            "name": "circuitKeys",
                            "outputs": [{"name": "alpha", "type": "tuple", "components": [{"type":"uint256"},{"type":"uint256"}]}], # Partial return match sufficient
                            "stateMutability": "view",
                            "type": "function"
                        }])
                        # Try to read key for circuit 0
                        checker.functions.circuitKeys(0).call()
                        logger.info("✅ Existing Sensor Verifier appears to be valid (Unified).")
                    except Exception as e:
                        logger.warning(f"⚠️ Existing Verifier at {current_verifier_addr} failed validation check (Old Type?): {e}")
                        logger.info("♻️ Triggering replacement with UnifiedGroth16Verifier...")
                        needs_update = True
    ...
    def _prediction_verifier_is_set(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Error comparing on-chain prediction keys with local keys: {e}")
                return True
        except Exception as e:
    ...
    def _upload_prediction_verifying_key(self) -> bool:
        ...
        try:
            ...
            def _norm(value): return HybridBlockchainHandler._norm_value(value)
            ...
            ) if self._gas_estimator else BlockchainConfig.VK_UPLOAD_GAS_LIMIT
    ...
    def _submit_prediction_proof_to_blockchain(self,
        ...
        try:
            ...
            if len(public_inputs_int) < 4:
                 return {'success': False, 'error': 'invalid_public_inputs_length_for_privacy'}

            prediction_commitment_int = public_inputs_int[3]
            prediction_commitment_bytes = prediction_commitment_int.to_bytes(32, byteorder='big')

            logger.info(f"🔍 PREDICTION PROOF: dataProofId={data_proof_id_onchain}, commitment={hex(prediction_commitment_int)[:20]}...")
            ...
            pred_gas_limit = getattr(BlockchainConfig, 'PREDICTION_GAS_LIMIT', 800000)
            ...
            eng_smart_acct = (self.role_wallets.get('ENGINEER') or {}).get('smart_account')
            eng_has_session = 'ENGINEER' in self._ephemeral_session_keys

            if eng_smart_acct and eng_has_session and ZKSYNC2_AVAILABLE:
                sa_result = self._send_as_smart_account(pred_contract_fn, eng_smart_acct, 'ENGINEER', pred_gas_limit)
            ...
            logger.info(f"✅ Prediction proof TX sent: {tx_hash.hex() if isinstance(tx_hash, (bytes, bytearray)) else tx_hash}")
            if receipt.status == 1:
                proof_id = self._extract_proof_id_from_receipt(receipt)
                logger.info(f"✅ Prediction proof submitted! proofId={proof_id}, block={receipt.blockNumber}")
                return {
                    'success': True,
                    'proof_id': proof_id,
                    'tx_hash': tx_hash.hex(),
    ...
    def submit_prediction_proof_automated(self, proof_data: dict) -> dict:
        ...
        """
        try:
            automation_key = self._normalize_raw_private_key(os.getenv("CHAINLINK_AUTOMATION_PRIVATE_KEY"))
            if not automation_key:
                logger.warning("Automation private key not configured, using admin wallet")
            ...
            model_commitment = proof_data.get('model_commitment') or self.web3.keccak(text="LSTM-CNN-v1.0")

            if isinstance(model_commitment, str):
                model_commitment = bytes.fromhex(model_commitment.replace('0x', ''))

            # Build transaction
            nonce = self.web3.eth.get_transaction_count(automation_account.address, 'pending')
            ...
            # Sign and send
            signed_tx = self.web3.eth.account.sign_transaction(tx, private_key=automation_key)
            tx_hash = self.web3.eth.send_raw_transaction(signed_tx.raw_transaction)
            ...
            if receipt.status == 1:
                proof_id = self._extract_proof_id_from_receipt(receipt)
                logger.info(f"✅ Automated prediction proof submitted! proofId={proof_id}")
                return {
                    'success': True,
                    'proof_id': proof_id,
                    'tx_hash': tx_hash.hex(),
    ...
    def prepare_sensor_proof(self, prediction_data: Dict, pdm_id=None) -> Dict:
        ...
        try:
            ...
            sensor_data = SensorData(
                machine_id=int(prediction_data.get('machine_id') or ((int(time.time()) % 9999) + 1)),
            ...
            data_id, data_hash = self.storage_manager.store_sensor_data(sensor_data, pdm_id=pdm_id)
            ...
            return {
                'success': True,
                'record_id': data_id,
                'contract_address': self.pdm_contract.address if self.pdm_contract else None,
                'proof_args': {
                    ...
                    'b': b,
                    'c': c,
                    'public_inputs': [str(x) for x in public_inputs_int] # String listesi olarak dön
                }
            }
    ...
    def submit_sensor_data_hybrid(self, prediction_data: Dict, pdm_id=None) -> Dict:
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"AttributeError during blockchain info update (likely optional DB missing): {ae}")
    ...
    def _submit_sensor_proof_to_blockchain(
        ...
        try:
            ...
            _op_smart_acct = (self.role_wallets.get('OPERATOR') or {}).get('smart_account')
            access_check_addr = (
            ...
            for i, b_try in enumerate(b_candidates):
                try:
                    ...
                    print(f"✅ Simulation passed with G2-Format-{i}! Gas: {gas_estimate}")
                    b = b_try
                    sim_ok = True
                    break
                except Exception as sim_err:
            ...
            gas_limit = getattr(BlockchainConfig, 'SENSOR_DATA_GAS_LIMIT', 800000)
            ...
            # Smart Account route (type-113) or EOA fallback
            smart_account_addr = (self.role_wallets.get('OPERATOR') or {}).get('smart_account')
            has_session_key = 'OPERATOR' in self._ephemeral_session_keys

            if smart_account_addr and has_session_key and ZKSYNC2_AVAILABLE:
                sa_result = self._send_as_smart_account(contract_fn, smart_account_addr, 'OPERATOR', gas_limit)
                tx_hash = bytes.fromhex(sa_result['tx_hash'].replace('0x', ''))
                receipt = sa_result['receipt']
            else:
                ...
                signed_tx = self.web3.eth.account.sign_transaction(submit_proof_tx, private_key=signer_pk)
                tx_hash = self.web3.eth.send_raw_transaction(signed_tx.raw_transaction)
                receipt = self.web3.eth.wait_for_transaction_receipt(tx_hash, timeout=BlockchainConfig.TRANSACTION_TIMEOUT)
    ...
    def _upload_circuit_vk(self, circuit_type_id: int, circuit_name: str) -> bool:
        """
        Verilen circuit'in VK'sini on-chain verifier'a yükle.
        circuit_name: 'fault_record_proof' | 'training_record_proof' | 'report_record_proof'
        circuit_type_id: 4 | 5 | 6  (UnifiedGroth16Verifier.CircuitType enum)
        """
        contract = self._get_verifier_contract()
        if not contract:
            return False
        ...
        if not vk_path.exists():
            ...
            if result.returncode != 0:
                logger.error(f"VK export başarısız ({circuit_name}): {result.stderr.strip()}")
                return False
        ...
        try:
            ...
            def _g2(p):
                return HybridBlockchainHandler._format_g2(p, swap=False)
            ...
            if receipt.status == 1:
                ...
                return True
            logger.error(f"VK yükleme TX başarısız (status=0): circuit {circuit_type_id}")
            return False
        except Exception as e:
    ...
    def _ensure_circuit_vk(self, circuit_type_id: int, circuit_name: str) -> bool:
        """VK on-chain set değilse otomatik yükle."""
    ...
    def submit_fault_record(
        ...
        try:
            ...
            result = self._build_and_send_tx(
                fn, signer_address, signer_pk,
                getattr(BlockchainConfig, 'FAULT_RECORD_GAS_LIMIT', 300000)
            )
            if result.get('success'):
                result['zk_proof_hash'] = '0x' + hashlib.sha256(
                    str(pub).encode()
                ).hexdigest()
            return result
    ...
    def submit_training_record(
        ...
        try:
            ...
            model_hash_bytes = (int(model_hash_int) % BN254_PRIME).to_bytes(32, byteorder='big')
    ...
    def _get_access_registry_contract(self):
        ...
        try:
            ...
            if not registry_address:
                logger.warning("AccessControlRegistry address not found in deployment info")
                return None

            # Load ABI
            artifact_path = Path("artifacts-zk/contracts/AccessControlRegistry.sol/AccessControlRegistry.json")
            if not artifact_path.exists():
                logger.warning(f"AccessControlRegistry artifact not found: {artifact_path}")
                return None
    ...
    def check_role(self, address: str, role: str) -> bool:
        ...
        try:
            ...
            if not contract:
                logger.warning("AccessControlRegistry not available, skipping on-chain role check")
    ...
    def get_node_info(self, address: str) -> Optional[Dict]:
        ...
        try:
            ...
            if self.admin_account:
                admin_node_ids = contract.functions.getNodesByAddress(self.admin_account).call()
    ...
    def _parse_node_info(self, node_id: bytes, node_info: tuple) -> Dict:
        ...
        return {
            ...
            'node_type': node_info[3],  # 0=UNDEFINED, 1=DATA_PROCESSOR, 2=FAILURE_ANALYZER, 3=MANAGER
    ...
    def check_resource_access(self, address: str, resource: str, required_level: int = 1) -> bool:
        """
        ...
        try:
            ...
            has_permission = contract.functions.nodePermissions(node_id, resource_hash).call()
    ...
    def verify_access(self, address: str, role: str = None, resource: str = None, required_level: int = 1) -> Dict:
        """
        ...
        try:
            ...
            if resource:
                result['has_resource_access'] = self.check_resource_access(address, resource, required_level)
    ...
    def register_node_on_blockchain(
        ...
        try:
            contract = self._get_access_registry_contract()
            if not contract:
                return {'success': False, 'error': 'AccessControlRegistry contract not available'}
            ...
            if existing_nodes:
                logger.info(f"User {user_address} already has {len(existing_nodes)} node(s) on blockchain")
                return {
                    'success': True,
            ...
            logger.info(f"Registering node on blockchain: {node_name} ({role}) for {checksum_addr}")
            ...
            if receipt.status == 1:
                ...
                logger.info(f"✅ User {checksum_addr} registered on blockchain successfully!")
                return {
                    'success': True,
                    'tx_hash': tx_hash.hex(),
                    'block_number': receipt.blockNumber,
...
try:
    ...
    def _patched_submit_sensor(self, sensor_data, data_hash, storage_location, proof_data, signer_address=None, signer_private_key=None):
