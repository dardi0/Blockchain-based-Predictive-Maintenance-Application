// scripts/set_verifying_key_prediction.js (Unified)
...
async function main(){
  ...
  const vkJsonPath = process.env.PREDICTION_VERIFICATION_KEY_JSON || path.join('C:', 'temp', 'zk_proofs', 'prediction_verification_key.json');
  ...
  if(!fs.existsSync(vkJsonPath)) throw new Error(`Prediction VK JSON yok: ${vkJsonPath}`);
  ...
  console.log('🚀 PREDICTION VK UnifiedGroth16Verifier\'a yazılıyor...');
  const tx = await verifier.setCircuitVerifyingKey(1, vk.alpha, vk.beta, vk.gamma, vk.delta, vk.IC);
  console.log('✅ TX:', tx.hash);
  await tx.wait();
  console.log('🎉 PREDICTION VK başarıyla ayarlandı.');
}

main().catch((e)=>{ console.error('❌ Prediction VK set hatası:', e); process.exit(1); });
